...
function validateEmploye(employe) {
    if (employe.nomEmploye!=="") { if (!rules.nom.test(employe.nomEmploye)) {
        throw new HttpError(400, "Format de nom invalide");
    }}
    if (employe.prenomEmploye!=="") { if (!rules.prenom.test(employe.prenomEmploye)) {
        throw new HttpError(400, "Format du prénom invalide");
    }	}
    if (employe.posteEmploye!=="") { if (!rules.posteEmploye.test(employe.posteEmploye)) {
        throw new HttpError(400, "Format du nom de poste invalide");
    }}
    if (employe.telephone!=='') { if (!rules.telephone.test(employe.telephoneEmploye)) {
        throw new HttpError(400, "Format du numéro de téléphone invalid");
    }}
    if (employe.numeroCivic!=='') { if (!rules.numeroCivic.test(employe.numeroCivic)) {
        throw new HttpError(400, "Format du numéro civic invalid");
    }}
    if (employe.numeroAppartement!=="") { if (!rules.numeroAppartement.test(employe.numeroAppartement)) {
        throw new HttpError(400, "Format du numéro d'appartement invalid");
    }}
    if (employe.nomRue!=="") { if (!rules.nomRue.test(employe.nomRue)) {
        throw new HttpError(400, "Format du nom de rue invalide");
    }}
    if (employe.nomVille!=="") { if (!rules.nomVille.test(employe.nomVillee)) {
        throw new HttpError(400, "Format du nom de la ville invalide");
    }}
    if (employe.nomProvince!=="") { if (!rules.nomProvince.test(employe.nomProvince)) {
        throw new HttpError(400, "Format du nom de la province invalide");
    }}
    if (employe.codePostal!=="") { if (!rules.codePostal.test(employe.codePostal)) {
